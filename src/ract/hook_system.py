...
class HookManager:
    ...
    base_dir: Path = field(default_factory=lambda: Path(".ract/hooks"))
    ...
    def run_hooks(
        ...
        env = os.environ.copy()
        env.update({f"RACT_{k.upper()}": str(v) for k, v in context.items()})
...
_plan = Plan(
    ...
    steps=[
        Step(
            ...
            expected_artifact="src/ract/hook_system.py",
        )
    ],
)
