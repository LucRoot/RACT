...
class PreflightValidator:
    """
    Validate a RACT configuration before starting work.
    """
...
_plan = Plan(
    assumption="preflight_validator must check that ract.yaml exists and contains required fields",
    confidence=0.95,
    steps=[
        Step(
            action="create PreflightValidator class with validate and is_valid methods",
            provider_hint="internal",
            expected_artifact="src/ract/preflight_validator.py",
        )
    ],
)
