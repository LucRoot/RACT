...
documented, tested starter project instead of a bare ``ract.yaml``.
"""
...
from ract.provider_presets import get_preset
...
class ProjectInitializer:
    ...
    LR:: Templates live under ``src/ract/project_templates/`` as JSON files.
    Each template declares the files to create and an optional built-in skill to
    install. The initializer refuses to overwrite an existing ``ract.yaml`` so
    ...
    def initialize(self) -> InitializationResult:
        ...
        config_path = self.project_dir / "ract.yaml"
