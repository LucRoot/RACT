...
class LoopController:
    ...
    def _persist_iteration_state(
        ...
        *,
        ...
        # cannot silently consume a stale / cross-run sidecar.
        #
        # SP amendment (Ox Alpha B Q7 DEFECT): delegate to
        # :func:`write_json_sidecar_with_header` so the atomicity
        # (tmp + os.replace), canonical serialization (dumps_jcs),
        # and ``sidecar.header.written`` event emission live in ONE
        # place. Previously this method hand-rolled all three,
        # which risked drift with any future change to the helper.
        try:
            ...
            return
        except Exception:  # noqa: BLE001 -- header helper best-effort
            # Fall through to a headerless legacy write when the
            # helper is unavailable (partial import) OR raises for
            # any other reason. The read side's legacy-fallback
            # path handles headerless payloads.
            try:
