...
def ensure_global_fixture_uniqueness(
    fixtures: List[Dict[str, Any]], user_story: str
) -> List[Dict[str, Any]]:
    """
    Ensure each fixture's ``pytest_fixture`` key is globally unique across the entire test suite.

    This function validates that the input list contains no duplicate ``pytest_fixture``
    values. If duplicates are found a :class:`ValueError` is raised before any transformation.
    Otherwise a new list is returned where each fixture's ``pytest_fixture`` value has an 8-
    character UUID suffix appended, preserving all other keys and values.
    """
