...
class SkillRegistry:
    """
    A registry for RootAct skills that stores templates and tool wrappers as JSON files.
...
_plan = Plan(
    assumption="RootAct must provide a SkillsRegistry that persists skills as JSON and supports register, load, list_skills, and invoke",
    confidence=0.95,
    steps=[
        Step(
            action="create SkillRegistry class with required methods",
            provider_hint="internal",
            expected_artifact="src/rootact/skills_registry.py",
        )
    ],
)
