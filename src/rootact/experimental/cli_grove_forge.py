...
def _grove_forge_command(args: list[str]) -> int:
    ...
    print(
        "[rootact] usage: ract grove-forge {eval|guardian} ...",
        file=sys.stderr,
    )
    return 1
...
def _grove_forge_eval_command(args: list[str]) -> int:
    ...
    if parsed.learning_feed:
        ...
        if not written:
            print(
                "[rootact] warning: learning feed could not be written",
                file=sys.stderr,
            )
...
def _grove_forge_guardian_command(args: list[str]) -> int:
    ...
    if parsed.output:
        if parsed.markdown_output:
            ...
            parsed.output.write_text(
                json.dumps(guardian_report_to_dict(report), indent=2, ensure_ascii=False),
                encoding="utf-8",
            )
