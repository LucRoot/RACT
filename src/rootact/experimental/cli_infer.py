...
``ract infer <task> --config rootact.yaml`` scores the task, selects the
...
def _infer_command(args: list[str]) -> int:
    ...
    parser.add_argument(
        ...
        help="Path to rootact.yaml with an inference_router section.",
    )
    parser.add_argument(
        "--json",
        dest="json_output",
    ...
    try:
        ...
        print(f"[rootact] missing dependency: {exc}", file=sys.stderr)
        return 1

    if parsed.live:
        print("[rootact] live inference is not yet supported via CLI", file=sys.stderr)
        return 2
    ...
    if not result.success:
        print(f"[rootact] routing failed: {result.error}", file=sys.stderr)
        return 1
