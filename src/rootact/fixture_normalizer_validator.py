...
def validate_and_normalize_fixtures(
    fixtures: List[Dict[str, Any]], user_story: str
) -> List[Dict[str, Any]]:
    """
    Validate and normalize pytest fixture names.
    ...
    This function ensures that each fixture's ``pytest_fixture`` value:
      ...
      - Does not collide with built-in Python identifiers
    ...
    for idx, fixture in enumerate(fixtures):
        ...
        if not re.fullmatch(r"test_[a-z_][a-z0-9_]*", raw_name):
            raise ValueError(
                f"Fixture name at index {idx} does not match pattern 'test_[a-z_][a-z0-9_]*' : '{raw_name}'"
            )
        ...
        # Check collision with built-in Python identifiers and keywords
        suffix_name = normalized_name[len("test_") :]
        if suffix_name in dir(builtins) or keyword.iskeyword(suffix_name):
            raise ValueError(
