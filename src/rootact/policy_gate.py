...
def evaluate_policy(policy: Dict[str, Any], evidence: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if min_quality is not None:
        ...
        if not receipts:
            ...
            if avg_score < min_quality:
                failures.append(f"Average quality score {avg_score:.2f} below threshold {min_quality}")
    ...
    if max_ratio is not None:
        ...
        if not changed_files:
            ...
            if ratio > max_ratio:
                failures.append(f"Unreceipted ratio {ratio:.2f} exceeds threshold {max_ratio}")
