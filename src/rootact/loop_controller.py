...
"""RootAct self-recursing build loop.

The LoopController runs a RootAct intent repeatedly, verifying each iteration
...
from rootact.preflight_test_validator import PreflightIssue, validate_report_tests
from rootact.progress_oracle import ROOT_KNOT, ProgressOracle
...
from rootact.test_failure_diagnoser import TestFailureDiagnoser
...
class LoopController:
    """Run a RootAct intent in a Root-Knot-anchored loop.

    The controller delegates each execution to ``run_rootact`` and then runs a
    ...
    def _run_with_timeout(self, intent: str) -> Rooted[ExecutionReport | Plan]:
        """Call ``run_rootact`` in a thread and enforce ``iteration_timeout``.
    ...
    def write_report(self, result: LoopResult, path: Path | str | None = None) -> Path:
        ...
        target = (
            Path(path) if path else self.project_dir / ".rootact" / "loop_report.json"
        )
