"""Ed25519 session keys for signing RootAct provenance capabilities."""
...
from rootact.core.types import Digest, digest_bytes
...
def _default_state_dir() -> Path:
    """Return the platform-specific XDG state directory for RootAct."""
    if os.name == "nt":
        local_appdata = os.environ.get("LOCALAPPDATA")
        if local_appdata:
            return Path(local_appdata) / "rootact"
        return Path.home() / "AppData/Local/rootact"
    xdg_state_home = os.environ.get("XDG_STATE_HOME")
    if xdg_state_home:
        return Path(xdg_state_home) / "rootact"
    return Path.home() / ".local/state/rootact"
