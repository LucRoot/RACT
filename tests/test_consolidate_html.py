...
from rootact.consolidate import ConsolidationResult, MergeProposal, render_html_report
...
def test_render_html_report_includes_proposal_and_consolidation():
    plan = ConsolidationResult(
        proposals=[
            MergeProposal(
                target="src/rootact/core.py",
                sources=("src/rootact/old_core.py",),
                diff="-old\n+new",
    ...
    assert "src/rootact/core.py" in html
