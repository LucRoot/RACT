...
def test_run_rootact_missing_config() -> None:
    result = run_rootact(Path("/nonexistent/rootact.yaml"), "intent")
    assert not result.is_ok()
    assert "not found" in (result.error or "").lower()


def test_run_rootact_invalid_config(tmp_path: Path) -> None:
    config_path = tmp_path / "rootact.yaml"
    config_path.write_text("invalid: [", encoding="utf-8")

    result = run_rootact(config_path, "intent")

    assert not result.is_ok()
    assert "preflight" in (result.error or "").lower()


def test_run_rootact_dry_run_success(tmp_path: Path) -> None:
    ...
    with patch(
        "rootact.rootact_runner.Harness.from_config_path",
        return_value=Rooted(value=mock_harness, assumption="loaded", confidence=1.0),
    ):
        result = run_rootact(config_path, "intent", dry_run=True)
...
def test_run_rootact_execution_success(tmp_path: Path) -> None:
    config_path = _write_config(tmp_path)
    ...
    with patch(
        ...
        with patch(
            "rootact.rootact_runner.enrich_harness_run",
...
def test_run_rootact_harness_init_failure(tmp_path: Path) -> None:
    config_path = _write_config(tmp_path)
...
def test_run_rootact_unsupported_mode(tmp_path: Path) -> None:
    config_path = _write_config(tmp_path)
    result = run_rootact(config_path, "intent", mode="invalid")
    assert not result.is_ok()
...
def test_run_rootact_documentation_mode_rewrites_intent(tmp_path: Path) -> None:
    config_path = _write_config(tmp_path)
    plan = Plan(assumption="test plan", confidence=0.9, steps=[Step("a", "b", "c")])
...
def test_run_rootact_saves_session_after_dry_run(tmp_path: Path) -> None:
    config_path = _write_config(tmp_path)
    plan = Plan(assumption="test plan", confidence=0.9, steps=[Step("a", "b", "c")])
    ...
    with patch(
        ...
        result = run_rootact(config_path, "intent", dry_run=True, session_id="s1")

    assert result.is_ok()
    session_file = tmp_path / ".rootact" / "sessions" / "s1.json"
    assert session_file.exists()


def test_run_rootact_resume_without_session_fails(tmp_path: Path) -> None:
    config_path = _write_config(tmp_path)
    result = run_rootact(config_path, "intent", resume=True)
    assert not result.is_ok()
    assert "requires --session" in (result.error or "")


def test_run_rootact_resume_loads_prior_session(tmp_path: Path) -> None:
    config_path = _write_config(tmp_path)
    from rootact.rootact_runner import _session_store_for

    store = _session_store_for(config_path)
    plan = Plan(assumption="prior", confidence=0.9, steps=[Step("a", "b", "c")])
    from rootact.session_store import SessionState
...
def test_run_rootact_overwrite_without_force_fails(tmp_path: Path) -> None:
    config_path = _write_config(tmp_path)
    from rootact.rootact_runner import _session_store_for
    from rootact.session_store import SessionState
    from dataclasses import asdict
    ...
    result = run_rootact(config_path, "new intent", session_id="s1")
    assert not result.is_ok()
    assert "already exists" in (result.error or "").lower()
    assert "--force" in (result.error or "") or "--resume" in (result.error or "")


def test_run_rootact_overwrite_with_force_succeeds(tmp_path: Path) -> None:
    config_path = _write_config(tmp_path)
    from rootact.rootact_runner import _session_store_for
    from rootact.session_store import SessionState
    from dataclasses import asdict
...
def test_run_rootact_resume_corrupted_session_fails(tmp_path: Path) -> None:
    config_path = _write_config(tmp_path)
    from rootact.rootact_runner import _session_store_for
    ...
    result = run_rootact(config_path, "intent", session_id="s1", resume=True)
    assert not result.is_ok()
    assert "corrupted" in (result.error or "").lower()


def test_run_rootact_rollback_without_session_fails(tmp_path: Path) -> None:
    config_path = _write_config(tmp_path)
    result = run_rootact(config_path, "intent", rollback=True)
    assert not result.is_ok()
    assert "requires --session" in (result.error or "")


def test_run_rootact_rollback_restores_snapshot(tmp_path: Path) -> None:
    config_path = _write_config(tmp_path)
    from rootact.session_rollback import SessionRollback
    ...
    result = run_rootact(config_path, "ignored", session_id="s1", rollback=True)
    assert result.is_ok()
    assert target.read_text(encoding="utf-8") == "original"


def test_run_rootact_rollback_missing_snapshot_fails(tmp_path: Path) -> None:
    config_path = _write_config(tmp_path)
    result = run_rootact(config_path, "ignored", session_id="s1", rollback=True)
    assert not result.is_ok()
...
def test_run_rootact_missing_project_doc_fails(tmp_path: Path) -> None:
    config_path = _write_config(tmp_path)
    result = run_rootact(config_path, "intent", project_doc=tmp_path / "missing.json")
    assert not result.is_ok()
    assert "project document" in (result.error or "").lower()


def test_run_rootact_project_doc_prefixes_intent(tmp_path: Path) -> None:
    config_path = _write_config(tmp_path)
    ...
    with patch(
        ...
        result = run_rootact(
            config_path, "add flag", dry_run=True, project_doc=doc_path
        )
...
def test_run_rootact_project_doc_updates_plan(tmp_path: Path) -> None:
    config_path = _write_config(tmp_path)
    doc_path = tmp_path / "doc.json"
...
def test_run_rootact_yolo_and_auto_are_mutually_exclusive(
    tmp_path: Path,
) -> None:
    config_path = _write_config(tmp_path)
    result = run_rootact(config_path, "intent", yolo=True, auto=True)
    assert not result.is_ok()
    assert "mutually exclusive" in (result.error or "").lower()


def test_run_rootact_yolo_passes_approval_callback(tmp_path: Path) -> None:
    config_path = _write_config(tmp_path)
    report = ExecutionReport(
        intent="test",
        step_results=[
    ...
    with patch(
        ...
        with patch(
            ...
            result = run_rootact(config_path, "intent", yolo=True)
...
def test_run_rootact_auto_passes_console_callback(tmp_path: Path) -> None:
    config_path = _write_config(tmp_path)
    report = ExecutionReport(
        intent="test",
        step_results=[
    ...
    with patch(
        ...
        with patch(
            ...
            result = run_rootact(config_path, "intent", auto=True)
...
def test_run_rootact_saves_memory_arena_for_session(tmp_path: Path) -> None:
    config_path = _write_config(tmp_path)
    step = Step(action="write", provider_hint="code", expected_artifact="src/foo.py")
    report = ExecutionReport(
        intent="test",
    ...
    with patch(
        ...
        with patch(
            ...
            result = run_rootact(config_path, "intent", session_id="mem_session")

    assert result.is_ok()
    memory_file = tmp_path / ".rootact" / "memory" / "mem_session.json"
...
def test_run_rootact_reload_executes_twice(tmp_path: Path) -> None:
    config_path = _write_config(tmp_path)
    report = ExecutionReport(
        intent="test",
        step_results=[
    ...
    with patch(
        ...
        with patch(
            ...
            result = run_rootact(config_path, "intent", reload=True)

    assert result.is_ok()
    assert mock_enrich.call_count == 2


def test_run_rootact_reload_second_run_fails(tmp_path: Path) -> None:
    config_path = _write_config(tmp_path)
    report = ExecutionReport(
        intent="test",
        step_results=[
...
def test_run_rootact_stream_callback_passed(tmp_path: Path) -> None:
    config_path = _write_config(tmp_path)
    report = ExecutionReport(
        intent="test",
        step_results=[
...
def test_run_rootact_rollback_with_missing_file(tmp_path: Path) -> None:
    config_path = _write_config(tmp_path)
    from rootact.session_rollback import SessionRollback
...
def test_run_rootact_default_mode_when_none(tmp_path: Path) -> None:
    config_path = _write_config(tmp_path)
    plan = Plan(assumption="plan", confidence=0.9, steps=[Step("a", "b", "c")])
    ...
    with patch(
        ...
        result = run_rootact(config_path, "intent", dry_run=True, mode=None)
...
def test_run_rootact_resume_includes_prior_artifacts(tmp_path: Path) -> None:
    config_path = _write_config(tmp_path)
    from rootact.rootact_runner import _session_store_for
    from rootact.session_store import SessionState
    from dataclasses import asdict
...
def test_run_rootact_captures_rollback_snapshot(tmp_path: Path) -> None:
    config_path = _write_config(tmp_path)
    target = tmp_path / "src" / "foo.py"
    target.parent.mkdir(parents=True, exist_ok=True)
    target.write_text("original", encoding="utf-8")
    ...
    with patch(
        ...
        with patch(
            ...
            result = run_rootact(config_path, "intent", session_id="s1")

    assert result.is_ok()
    assert captured_plan is not None
    snapshot_path = tmp_path / ".rootact" / "snapshots" / "s1.json"
    assert snapshot_path.is_file()


def test_run_rootact_project_doc_updates_after_execution(tmp_path: Path) -> None:
    config_path = _write_config(tmp_path)
    doc_path = tmp_path / "doc.json"
    doc_path.write_text('{"goal": "Build a CLI", "plan": []}', encoding="utf-8")
    step = Step(action="write", provider_hint="code", expected_artifact="src/foo.py")
    ...
    with patch(
        ...
        with patch(
            ...
            result = run_rootact(config_path, "add flag", project_doc=doc_path)
