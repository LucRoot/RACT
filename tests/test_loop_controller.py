...
def test_loop_stops_on_done_callback(tmp_path):
    ...
    with patch(
        ...
        with patch.object(controller, "_run_tests", return_value=(0, "2 passed", "")):
            result = controller.run("add a feature", done_callback=lambda _it: True)
...
def test_loop_blocks_done_on_refactor_tax_breach(tmp_path):
    ...
    with patch("ract.loop_controller.run_ract", side_effect=_write_and_return):
        with patch.object(controller, "_run_tests", return_value=(0, "2 passed", "")):
            result = controller.run("add a feature", done_callback=lambda _it: True)
    assert len(result.iterations) == 1
    assert result.final_decision == "stop"
...
def test_loop_detects_quality_regression(tmp_path):
    ...
    config_path.write_text("project:\n  name: test\n", encoding="utf-8")
    controller = LoopController(config_path, max_iterations=10)
    first = _make_report({"src/foo.py": "# code\n# existing\n"}, project_dir=tmp_path)
...
def test_loop_uses_custom_test_command(tmp_path):
    ...
    controller = LoopController(
        config_path, max_iterations=1, test_command=["-m", "pytest", "-q", "--tb=short"]
    )
    report = _make_report({})
    with patch(
        "ract.loop_controller.run_ract",
...
def test_loop_attempts_repair_when_tests_fail(tmp_path):
    config_path = tmp_path / "ract.yaml"
    config_path.write_text("project:\n  name: test\n", encoding="utf-8")
    controller = LoopController(config_path, max_iterations=2, repair_attempts=1)
    report = _make_report({"src/foo.py": "# code\n# existing\n"}, project_dir=tmp_path)
    failing_output = (
    ...
    with patch(
        ...
        with patch.object(
            controller,
            "_run_tests",
            side_effect=[(1, "1 failed", failing_output), (0, "1 passed", "")],
        ):
            result = controller.run("add a feature")
...
def test_error_memory_included_in_augmented_intent(tmp_path):
    ...
    controller._error_memory.record(
        SimpleNamespace(
            index=1,
            test_output="FAILED tests/test_foo.py::test_one - AssertionError: assert 1 == 2",
            error="",
            reflection="",
        )
    )
...
def test_strategic_context_clear_after_stagnant_streak(tmp_path):
    ...
    config_path.write_text("project:\n  name: test\n", encoding="utf-8")
    controller = LoopController(
        config_path, max_iterations=6, strategic_clear_threshold=2
    )
    ...
    with patch(
        ...
        with patch.object(
            ...
            result = controller.run(
                "add a feature", done_callback=lambda it: it.index == 4
            )
    assert result.final_decision == "done"
...
def test_preflight_repair_skips_pytest_for_invalid_test(tmp_path):
    config_path = tmp_path / "ract.yaml"
    config_path.write_text("project:\n  name: test\n", encoding="utf-8")
    controller = LoopController(config_path, max_iterations=3, repair_attempts=1)
    bad_test = "def test_foo():\n    assert re.match('x', 'x')\n# existing\n"
    fixed_test = (
        "import re\ndef test_foo():\n    assert re.match('x', 'x')\n# existing\n"
    )
    bad_report = _make_report({"tests/test_foo.py": bad_test}, project_dir=tmp_path)
    fixed_report = _make_report({"tests/test_foo.py": fixed_test}, project_dir=tmp_path)
    with patch(
        "ract.loop_controller.run_ract",
        side_effect=[
        ...
        with patch.object(
            ...
            result = controller.run(
                "add a feature", done_callback=lambda it: it.test_returncode == 0
            )
    assert result.iterations[0].decision == "continue"
...
def test_loop_handshakes_high_risk_milestone(tmp_path):
    ...
    controller = LoopController(
        config_path, max_iterations=2, planner=planner, milestone_oracle=oracle
    )
    existing = tmp_path / "src" / "existing.py"
    existing.parent.mkdir(parents=True, exist_ok=True)
    existing.write_text("# existing\n# existing\n", encoding="utf-8")
...
def test_loop_regression_on_milestone_oracle_failure(tmp_path):
    ...
    oracle = MagicMock(spec=MilestoneOracle)
    oracle.evaluate.return_value = Rooted(
        value=None, assumption="ok", confidence=0.0, error="oracle failed"
    )
    controller = LoopController(
        config_path, max_iterations=2, planner=planner, milestone_oracle=oracle
    )
...
def test_loop_uses_string_test_command(tmp_path):
    ...
    config_path.write_text("project:\n  name: test\n", encoding="utf-8")
    controller = LoopController(
        config_path, max_iterations=1, test_command="-m pytest -q"
    )
...
def test_take_snapshot_skips_pycache_and_handles_errors(tmp_path):
    ...
    src = tmp_path / "src"
    src.mkdir()
    (src / "clean.py").write_text("# existing\n", encoding="utf-8")
...
def test_preflight_repair_exhausted_returns_regression(tmp_path):
    config_path = tmp_path / "ract.yaml"
    config_path.write_text("project:\n  name: test\n", encoding="utf-8")
    controller = LoopController(config_path, max_iterations=2, repair_attempts=0)
    bad_test = "def test_foo():\n    assert re.match('x', 'x')\n# existing\n"
    bad_report = _make_report({"tests/test_foo.py": bad_test}, project_dir=tmp_path)
    with patch(
        "ract.loop_controller.run_ract",
...
def test_stagnation_limit_triggers_stop(tmp_path):
    config_path = tmp_path / "ract.yaml"
    config_path.write_text("project:\n  name: test\n", encoding="utf-8")
    controller = LoopController(config_path, max_iterations=10, stagnation_limit=2)
    stuck_report = _make_report(
        {"src/foo.py": "# code\n# existing\n"}, project_dir=tmp_path
    )
    with patch(
...
def test_milestone_verdict_wrong_knot_returns_regression(tmp_path):
    ...
    oracle.evaluate.return_value = Rooted(
        value=SimpleNamespace(
            verdict="proceed", reason="ok", confidence=1.0, knot=object()
        ),
        assumption="ok",
        confidence=1.0,
    )
...
def test_quality_floor_triggers_regression(tmp_path):
    config_path = tmp_path / "ract.yaml"
    config_path.write_text("project:\n  name: test\n", encoding="utf-8")
    controller = LoopController(config_path, max_iterations=1, quality_floor=0.99)
    report = _make_report({"src/foo.py": "# code\n# existing\n"}, project_dir=tmp_path)
    with patch(
        "ract.loop_controller.run_ract",
...
def test_attempt_lint_repair_bad_prompt_rooted_returns_false(tmp_path):
    ...
    with patch.object(controller._lint_repair, "check") as mock_check:
        ...
        with patch.object(
            ...
            "build_repair_prompt",
            return_value=Rooted(value=None, error="bad", assumption="", confidence=0.0),
        ):
            assert not controller._attempt_lint_repair()
...
def test_attempt_repair_returns_false_when_diagnoser_fails(tmp_path):
    ...
    with patch.object(
        ...
        "diagnose",
        return_value=Rooted(
            value=None, error="could not diagnose", assumption="", confidence=0.0
        ),
    ):
        assert not controller._attempt_repair(iteration)
